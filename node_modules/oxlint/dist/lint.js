...
function deserializeFormalParameters(pos) {
	...
	if (uint32[pos + 32 >> 2] !== 0 && uint32[pos + 36 >> 2] !== 0) {
		...
		rest.argument = deserializeBindingPattern(pos + 40), rest.typeAnnotation = deserializeOptionBoxTSTypeAnnotation(pos + 56), rest.typeAnnotation !== null && (end = rest.typeAnnotation.end, rest.end = end, rest.range[1] = end), params.push(rest), parent = previousParent;
	}
	return params;
}
...
function createContext(ruleDetails) {
	return ObjectFreeze({
		...
		report(diagnostic) {
			report(normalizeReportCallArgs(diagnostic, arguments), ruleDetails);
		}
	});
}
