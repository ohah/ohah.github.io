import "./bindings-CLlZ-RUP.js";
import "./apis-CABy38Rv.js";
...
const textToDoc = async (embeddedSourceText, textToDocOptions) => {
	...
	let { code, errors } = await format(parser === "typescript" ? filepath.endsWith(".tsx") ? "dummy.tsx" : "dummy.ts" : "dummy.jsx", embeddedSourceText, JSON.parse(_oxfmtPluginOptionsJson));
	if (0 < errors.length) throw new Error(errors[0].message);
	if (parentParser === "markdown" || parentParser === "mdx") {
		if (code.startsWith("<") && code.endsWith(">;")) code = code.slice(0, -1);
		if (code.startsWith(";<") && code.endsWith(">")) code = code.slice(1);
	}
