...
버전 표는 2026년 9월 26일 확인한 공개 `main`의 의존성 선언이다. 실제 잠금 파일의 해석 버전이나 v0.1.2 릴리즈의 의존성 목록과는 다를 수 있다. 근거는 [Rust 워크스페이스](https://github.com/ohah/cheolsu-proxy/blob/2236ff77/Cargo.toml)와 각 크레이트의 `Cargo.toml`, [데스크톱 package.json](https://github.com/ohah/cheolsu-proxy/blob/2236ff77/desktop/package.json)이다.
...
| **tokio-rustls** | Rustls 기반 TLS 처리 | 0.26.4 |
| **rcgen** | 동적 인증서 생성 | 0.14 |
| **tokio-tungstenite** | WebSocket 프로토콜 | 0.28.0 |
...
| 라이브러리 | 역할 | 버전 |
|------------|------|------|
| **deno_core** | V8 JavaScript 런타임 | 0.400 |
| **v8** | JavaScript 엔진 | 147 |
| **oxc_parser** | JS/TS 파서 | 0.128 |
| **oxc_transformer** | AST 변환 | 0.128 |
| **oxc_codegen** | 코드 생성 | 0.128 |
...
| 도구 | 역할 | 버전 |
|------|------|------|
| **Tauri** | 데스크톱 프레임워크(Rust 크레이트) | 2.11.5 |
| **React** | UI 라이브러리 | ^19.2.8 |
| **Zustand** | 상태 관리 | ^5.0.14 |
| **React Hook Form + Zod** | 폼 + 검증 | ^7.77.0 / ^4.4.3 |
| **@monaco-editor/react** | 코드 에디터 React 래퍼 | ^4.7.0 |
| **Tailwind CSS** | 스타일링 | ^4.3.0 |
| **@lingui/react** | i18n | 6.2.0 |
| **Vite** | 번들러 | ^8.0.16 |
