...
레퍼런스로 삼은 [devtools-remote-debugger](https://github.com/Nice-PLQ/devtools-remote-debugger)와 [chobitsu](https://github.com/liriliri/chobitsu)에 이미 구현이 되어 있었다. 콘솔이나 네트워크 후킹은 사실 과거에 조금 만져본 적이 있었고, CDP 프로토콜 구조도 대충은 알고 있었기 때문에 AI에게 레퍼런스 코드를 던져주고 "이거 기반으로 짜줘"라고 하니까 금방 나왔다.

내가 잘 아는 영역이라 AI의 출력을 검증하기도 쉬웠다. 잘 모르는 영역에서 AI를 쓰면 질문-검증 사이클이 길어지는데, 이건 그런 게 없어서 속도가 빨랐다.
