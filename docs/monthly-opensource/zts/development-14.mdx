...
[3-13](development-13)의 마지막 — 트리쉐이킹 에픽 시작 + "statement-level symbol graph 완벽히 구현되면 필요 없다"는 인식 — 이 편의 출발점이다. 같은 정보를 여러 자료구조가 중복 보관하는 현재 구조를 정리하고, AST 자체를 oxc 스타일로 재설계하는 쪽으로 방향이 잡혀 있다.
